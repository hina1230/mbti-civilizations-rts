...
{
    public class CivilizationManagerBasic : MonoBehaviour
    {
        ...
        private Dictionary<string, CivilizationBaseBasic> activeCivilizations;
        ...
        private void Awake()
        {
            ...
            activeCivilizations = new Dictionary<string, CivilizationBaseBasic>();
        ...
        private void InitializeCivilizationData()
        {
            ...
            {
                var civBase = prefab.GetComponent<CivilizationBaseBasic>();
                if (civBase != null)
                {
        ...
        public void InitializeCivilization(CivilizationType type)
        {
            ...
            SpawnCivilization(localPlayerId, type);
        }

        private void SpawnCivilization(string playerId, CivilizationType type)
        {
            Debug.Log($"Player {playerId} initialized as {type} civilization");
            
            // For now, just create a basic civilization without instantiating prefabs
            // This can be expanded when prefabs are properly set up
        }

        public CivilizationBaseBasic GetPlayerCivilization(string playerId)
        {
            return activeCivilizations.ContainsKey(playerId) ? activeCivilizations[playerId] : null;
        }
    ...
    // Basic civilization base class without Netcode dependency
    public class CivilizationBaseBasic : MonoBehaviour
    {
        ...
        {
            Debug.Log($"Initialized {civilizationType} civilization for {playerId}");
        }
    }
